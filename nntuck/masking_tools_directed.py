...
#------ Import Libraries --------#
import nntuck.NNTucktools  #this .py file contains function definitions for non_negative_tucker with KL loss,
                           # as well as for MULTITENSOR's EM updates
import numpy as np
from nntuck.NNTucktools import non_negative_tucker, non_negative_tucker_ones
...
def Per_alpha(tensor, c, K, Maskings = None):
    # for Y of dimension (L x c) sweep over dimension c
        ...
        # object of size (4):(current_AUC, current_like (train likelihood), ten, M)
        # each item in max_results is the info for the NTD with the highest log like/AUC for each M over 50 runs
    ...
    with parallel_backend('multiprocessing'):
        Big_Results = Parallel(n_jobs = 4)(delayed(Per_j)(j, Maskings, tensor, c, K, symm = False) for j in range(num_it))
    ...
    for i in range(num_it):
        for k in range(5):
            likelihoods[i, k] = Big_Results[i][k][1]
            AUCs[i, k] = Big_Results[i][k][0]
    max_results = []
    goal = likelihoods # change to AUCs if you want to pick the decomp with the highest test AUC
    for k in range(5):
        max_idx = np.argmax(goal[:, k])
        max_results.append(Big_Results[max_idx][k])
    return max_results 
...
def Per_alpha_c(tensor, K, Maskings = None):
    ...
    if Maskings is None:
        Maskings = masking_tensor(tensor)
    with parallel_backend('multiprocessing'):
        Big_Results = Parallel(n_jobs = 4)(delayed(Per_j_c)(j, Maskings, tensor, c, K, symm = False) for j in range(num_it))
    #Big_Results is (num_it x 5) where each entry is an object of size (4):(current_AUC, current_like, ten, M)
...
def Per_alpha_ones(tensor, K, Maskings= None):
    # for Y == I
        # define a set of 5 masking tensors
        # then for each M_i in this set:
            # return the decompositon associated with the highest likelihood over num_it runs
    ...
    if Maskings is None:
        Maskings = masking_tensor(tensor)
    with parallel_backend('multiprocessing'):
        Big_Results = Parallel(n_jobs = 4)(delayed(Per_j_ones)(j, Maskings, tensor, c, K, symm = False) for j in range(num_it))
    #Big_Results is (num_it x 5) where each entry is an object of size (4):(current_AUC, current_like, ten, M)
...
def Per_j(j, Maskings, tensor, C, K, symm = False):
    ...
    if j%5==0:
        print('j = ', j)
    C_Results = Parallel(n_jobs=1)(delayed(TuckerAUC_masked)(M, tensor, C, K, symm= False) for M in Maskings)
    return C_Results
...
def Per_j_c(j, Maskings, tensor, C, K, symm = False):
    ...
    if j%5==0:
        print('j = ', j)
    C_Results = Parallel(n_jobs=1)(delayed(TuckerAUC_masked_c)(M, tensor, ALPHA, K, symm= False) for M in Maskings)
    return C_Results
...
def Per_j_ones(j, Maskings, tensor, C, K, symm = False):
    ...
    if j%5==0:
        print('j = ', j)
    C_Results = Parallel(n_jobs=1)(delayed(TuckerAUC_masked_ones)(M, tensor, 1, K, symm= False) for M in Maskings)
    return C_Results
