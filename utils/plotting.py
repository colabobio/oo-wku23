...
def color_graph_by_attribute(G, pos, attrib, title, weights, cmap=plt.cm.Oranges):
    ...
    cleaned_attrib = [np.nan if pd.isna(x) else x for x in attrib]    
    colors = cmap(cleaned_attrib)
    # Plot the graph
    plt.figure(figsize=(10, 10))  # Set the figure size
    nx.draw_networkx(G, pos, node_color=cleaned_attrib, cmap=cmap, with_labels=False, node_size=7, width=weights, alpha=1)
