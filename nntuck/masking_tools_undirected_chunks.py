...
def masking_tensor_undirected_chunks(tensor): 
    # This function returns a masking tensor for a **directed** network (e.g., M == M.T)
        # here, M_ijk == 0 means that A_ijk is _unobserved_ 
        # In each of the M's with uniform probability a 
        # random set of ~20% of the entries are unobserved
    ...
    a = np.where(np.logical_and(np.triu(M_fold[0]) <= 0.2, np.triu(M_fold[0]) > 0.0))
    M_1[:, a[0], a[1]] = 0
    M_1[:, a[1], a[0]] = 0

    a = np.where(np.logical_and(np.triu(M_fold[0]) <= 0.4, np.triu(M_fold[0]) > 0.2))
    M_2[:, a[0], a[1]] = 0
    M_2[:, a[1], a[0]] = 0

    a = np.where(np.logical_and(np.triu(M_fold[0]) <= 0.6, np.triu(M_fold[0]) > 0.4))
    M_3[:, a[0], a[1]] = 0
    M_3[:, a[1], a[0]] = 0

    a = np.where(np.logical_and(np.triu(M_fold[0]) <= 0.8, np.triu(M_fold[0]) > 0.6))
    M_4[:, a[0], a[1]] = 0
    M_4[:, a[1], a[0]] = 0
    
    a = np.where(np.logical_and(np.triu(M_fold[0]) <= 1, np.triu(M_fold[0]) > 0.8))
    M_5[:, a[0], a[1]] = 0
    M_5[:, a[1], a[0]] = 0
...
def Per_alpha(tensor, c, K):
    ...
    Maskings = masking_tensor_undirected_chunks(tensor) #malaria is undirected. 
    num_it = NUM_IT
...
def Per_alpha_c(tensor, K):
    ...
    num_it = NUM_IT
    print("doing Y == I")
    Maskings = masking_tensor_undirected_chunks(tensor)
    with parallel_backend('multiprocessing'):
